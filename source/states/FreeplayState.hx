...
class FreeplayState extends MusicBeatState
{
	...
	override function update(elapsed:Float)
	{
		...
		if (controls.ACCEPT)
		{
			...
			stopPreview();
			openSubState(new substates.StickersSubState(PlayState));
		}
	}
	...
	function playPreview():Void
	{
		...
		var songLowercase:String = songs[curSelected].songName.toLowerCase();
		var instSound:openfl.media.Sound = Paths.inst(songLowercase);
		
		if (instSound != null)
		{
			instPlaying = FlxG.sound.play(instSound, 0.7);
			...
			audioVisualizer = new AudioVisualizer(FlxG.width * 0.5 - 256, FlxG.height - 220, instPlaying);
			add(audioVisualizer);
		}
	}
