...
import data.song.ChartConverter;
...
class PlayState extends scripting.ScriptableState
{
	...
	/** Expose songPosition for scripts */
	...
	// ── ScriptableState override — expose `game` = PlayState.instance ──────
	...
	override public function create()
	{
		...
		ScriptManager.loadFolder("mods/scripts/songs/" + (SONG != null ? SONG.songId.toLowerCase() : ""));
		// Also check assets/data/<songId>/scripts/
		if (SONG != null)
			ScriptManager.loadFolder("assets/data/" + SONG.songId.toLowerCase() + "/scripts");
		// Window config (if present)
		var winCfg = "assets/data/window.json";
		#if FEATURE_FILESYSTEM
		if (FileSystem.exists(winCfg))
			ScriptManager.loadScript(winCfg);
		#end
		...
		// ── Inject initial globals into scripting system ──────────────────────
		...
		super.create();
		// Fire song-start on state-specific scripts (loaded by ScriptableState.create above)
	...
	var debugNum:Int = 0;
	/** Debug overlay text for script trace() output */
	public var debugText:flixel.text.FlxText = null;
	...
	override public function update(elapsed:Float)
	{
		...
		// Reactive audio + adaptive AI update
		if (songStarted)
		{
			reactiveAudio.update(accuracy, combo, health / 2.0, elapsed);
			adaptiveAI.update(accuracy, combo, health, elapsed);
		}

		// Cinematic camera lerp back
		...
		if (useVideo && GlobalVideo.get() != null && !stopUpdate)
		{
			...
			{
				#if FEATURE_VIDEO
				if (videoHandler != null) remove(videoHandler.sprite);
				#end
				removedVideo = true;
			}
		}
		...
		if (health <= 0 && !cannotDie)
		{
			if (!usedTimeTravel)
			{
				...
				// ── Script callback ───────────────────────────────────────────
				ScriptManager.call(ScriptCallbacks.ON_GAME_OVER, []);
				scripts.call(ScriptCallbacks.ON_GAME_OVER, []);
	...
	function endSong():Void
	{
		...
		if (useVideo)
		{
			...
			if (PlayState.instance.videoHandler != null)
				PlayState.instance.remove(PlayState.instance.videoHandler.sprite);
			#end
		}
	...
	public function backgroundVideo(source:String):Void
	{
		#if FEATURE_VIDEO
		useVideo     = true;
		removedVideo = false;
		...
		videoHandler.source(source);
		videoHandler.makePlayer();

		// hxvlc renders via FlxVideoSprite — position it before adding
		videoHandler.sprite.x = GlobalVideo.calc(0);
		videoHandler.sprite.y = GlobalVideo.calc(1);
		videoHandler.sprite.setGraphicSize(
			Std.int(GlobalVideo.calc(2)),
			Std.int(GlobalVideo.calc(3))
		);
		videoHandler.sprite.updateHitbox();

		// Callback so the update loop knows the video ended
		videoHandler.onEnd = function() { removedVideo = true; };

		GlobalVideo.setVid(videoHandler);

		// Re-layer: video behind characters
		add(videoHandler.sprite);
		remove(gf);
		remove(boyfriend);
		remove(dad);
		add(gf);
	...
	/**
	 * triggerEventNote — routes ALL chart events through EventSystem.
	 * Scripts can intercept, modify, or add new event types.
	 * Called from update() wherever the original inline event logic was.
	 */
	public function triggerEventNote(name:String, value1:String = "", value2:String = ""):Void
	{
		EventSystem.fire(name, value1, value2);
	}
	...
	/**
	 * addTextToDebug — prints a debug message to the HUD overlay.
	 * Called by HaxeScriptBackend trace() when PlayState is active.
	 * Falls back to haxe.Log.trace() if debugText is unavailable.
	 */
	public function addTextToDebug(msg:String, color:flixel.util.FlxColor):Void
	{
		if (debugText != null)
		{
			debugText.text = msg + "\n" + debugText.text;
			// Keep at most 20 lines to prevent overflow
			var lines = debugText.text.split("\n");
			if (lines.length > 20) debugText.text = lines.slice(0, 20).join("\n");
		}
		else
			haxe.Log.trace("[Script] " + msg, null);
	}
	...
	override public function destroy():Void
	{
		...
		EventSystem.destroy();
		if (reactiveAudio != null) reactiveAudio.stop();
		ShaderManager.clearCamera(camGame);
		ShaderManager.clearCamera(camHUD);
		instance = null;
		super.destroy();
